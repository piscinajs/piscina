...
for (const QueueClass of [FixedQueue, ArrayTaskQueue]) {
  test(`${QueueClass.name} - unshift`, () => {
    ...
    // Edge case: unshift into an empty queue
    // In the FixedQueue, this appends a new FixedCircularBuffer
    queue.unshift(new NumberedTask(1));
    assert.equal(queue.size, 1);

    // Unshifting more tasks (more than the FixedCircularBuffer size)
    // This also tests unshifting when one of the buffers is full
    for (let i = 2; i <= bufferSize + 10; ++i) {
      queue.unshift(new NumberedTask(i));
    }
    assert.equal(queue.size, bufferSize + 10);

    // Shift 5 tasks => returns tasks in the reverse order
    for (let i = 0; i < 5; ++i) {
      assert.equal((queue.shift() as NumberedTask).index, bufferSize + 10 - i);
    }
    assert.equal(queue.size, bufferSize + 5);

    // Shift all the remaining tasks
    for (let i = 0; i < bufferSize + 5; ++i) {
      assert.equal((queue.shift() as NumberedTask).index, bufferSize + 5 - i);
    }
    assert.equal(queue.size, 0);

    // Pushing tasks puts them to the end of the queue, whereas unshifting inserts them at the beginning
