...
test('will reject items when task queue is unavailable', async ({ equal, rejects, fail, match }) => {
  const pool = new Piscina({
    filename: resolve(__dirname, 'fixtures/eval.js'),
    minThreads: 0,
    maxThreads: 1,
  ...
  try {
    const p = pool.run('while (true) {}')

    equal(pool.threads.length, 1);
    equal(pool.queueSize, 0);

    try {
      await pool.run('while (true) {}')
      fail("should have rejected")
    } catch (e) {
      match(e.message, /No task queue available and all Workers are busy/)
    }
    ...
    match(e.message, /Terminating worker thread/)
  }
