...
Piscina supports transferring `MessagePort` objects to workers, allowing for direct communication between the main thread and worker threads. This can be particularly useful for scenarios where you need ongoing communication or to send large amounts of data efficiently between threads.

This example demonstrates how to use `MessagePort` with Piscina.
