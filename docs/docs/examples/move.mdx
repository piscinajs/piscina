...
Piscina provides a `move()` function that allows the transfer of data between the main thread and worker threads. The example below will sohw how to use `Piscina.move()` to transfer `ArrayBuffer` without cloning, which can significantly improve performance for large data transfers.
...
The worker file also uses `move()` to transfer data back to the main thread:
